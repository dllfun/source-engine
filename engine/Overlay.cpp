								         ...
								         SurfaceHandle_t surfID )
{
	...
	mtexinfo_t *pTexInfo = &host_state.worldmodel->brush.pShared->texinfo[pOverlay->m_nTexInfo];
...
void COverlayMgr::Surf_CreateFragments( moverlay_t *pOverlay, SurfaceHandle_t surfID )
{
	...
	for( int iTri = 0; iTri < nSurfTriangleCount; ++iTri )
	{
		...
		int iVert = host_state.worldmodel->brush.pShared->vertindices[(iFirstVert)];
		mvertex_t *pVert = &host_state.worldmodel->brush.pShared->vertexes[iVert];
		surfaceFrag.m_aPrimVerts[0].pos = pVert->position;
		
		iVert = host_state.worldmodel->brush.pShared->vertindices[(iFirstVert+iTri+1)];
		pVert = &host_state.worldmodel->brush.pShared->vertexes[iVert];
		surfaceFrag.m_aPrimVerts[1].pos = pVert->position;
		
		iVert = host_state.worldmodel->brush.pShared->vertindices[(iFirstVert+iTri+2)];
		pVert = &host_state.worldmodel->brush.pShared->vertexes[iVert];
								        ...
								        SurfaceHandle_t surfID )
{
	...
	for( int iVert = 0; iVert < 4; ++iVert )
	{
		int iVertex = host_state.worldmodel->brush.pShared->vertindices[(iFirstVert+iVert)];
		mvertex_t *pVert = &host_state.worldmodel->brush.pShared->vertexes[iVertex];
