...
dworldlight_t* FindAmbientLight()
{
	...
	for (int i = 0; i < g_pHost->host_state.worldmodel->brush.pShared->numworldlights; i++)
	{
		if (g_pHost->host_state.worldmodel->brush.pShared->worldlights[i].type == emit_skyambient)
		{
			return &g_pHost->host_state.worldmodel->brush.pShared->worldlights[i];
		}
	}
	...
	const Vector &vecNormal, IHandleEntity *pIgnoreEnt, Vector *pLinearColor )
{
	...
	{
		dworldlight_t *wl = &g_pHost->host_state.worldmodel->brush.pShared->worldlights[i];
	...
	bool &bReject )
{
	...
	if( !bIgnoreVis )
	{
		...
		{
 			int  bucketOriginLeaf = CM_PointLeafnum( bucketOrigin );
			mleaf_t *pLeaf = &g_pHost->host_state.worldmodel->brush.pShared->leafs[bucketOriginLeaf];
...
static void AddLightStylesForStaticProp( PropLightcache_t *pcache, LightingState_t& lightingState )
{
	...
	for( int i = 0; i < pcache->m_LightStyleWorldLights.Count(); ++i )
	{
		...
		dworldlight_t *wl = &g_pHost->host_state.worldmodel->brush.pShared->worldlights[pcache->m_LightStyleWorldLights[i]];
		Assert( wl->style != 0 );
	...
	bool bAddedLeafAmbientCube )
{
	...
	for (i = 0; i < g_pHost->host_state.worldmodel->brush.pShared->numworldlights; ++i)
	{
		...
		if ( r_lightcache_zbuffercache.GetInt() )
			pZBuf = &g_pHost->host_state.worldmodel->brush.pShared->shadowzbuffers[i];
		else
			pZBuf = NULL;
...
static void BuildStaticLightingCacheLightStyleInfo( PropLightcache_t* pcache, const Vector& mins, const Vector& maxs )
{
	...
	for ( short i = 0; i < g_pHost->host_state.worldmodel->brush.pShared->numworldlights; ++i)
	{
		dworldlight_t *wl = &g_pHost->host_state.worldmodel->brush.pShared->worldlights[i];
		if (wl->style == 0)
			continue;
