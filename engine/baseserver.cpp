							    ...
							    const char *name, const char *password, const char *hashedCDkey, int cdKeyLen )
{
	...
	INetChannel * netchan = GetNetSocket()->NET_CreateNetChannel( &adr, adr.ToString(), client );
...
bool CBaseServer::GetClassBaseline( ServerClass *pClass, void const **pData, int *pDatalen )
{
	if ( sv_instancebaselines.GetInt() )
	{
		ErrorIfNot( pClass->GetInstanceBaselineIndex() != INVALID_STRING_INDEX,
			("SV_GetInstanceBaseline: missing instance baseline for class '%s'", pClass->GetNetworkName())
		);
...
void CBaseServer::ForwardPacketsFromMasterServerUpdater()
{
	...
	while ( 1 )
	{
		...
		GetNetSocket()->NET_SendPacket( NULL, adr, packetData, len );
	}
}
...
CBaseClient *CBaseServer::CreateFakeClient( const char *name )
{
	...
	if ( sv_stressbots.GetBool() )
	{
		...
		netchan = GetNetSocket()->NET_CreateNetChannel( &adrNull, adrNull.ToString(), fakeclient, true );
	}
