...
bool CDispInfo::Render( CGroupMesh *pGroup, bool bAllowDebugModes )
{
	...
	if ( bAllowDebugModes )
	{
		...
		if ( mat_surfaceid.GetInt() )
		{
			...
			intp nInt = ( mat_surfaceid.GetInt() != 2 ) ? (intp)m_ParentSurfID : (SurfaceHandle_t)m_ParentSurfID - host_state.worldmodel->brush.pShared->surfaces2;
			char buf[32];
						...
						Vector2D* luv, Vector2D* tuv )
{
	...
	int idx = DispInfo_ComputeIndex( host_state.worldmodel->brush.pShared->hDispInfos, this );
