...
bool EffectOccluded( const Vector &pos, pixelvis_handle_t *queryHandle )
{
	if ( !queryHandle )
	{
		...
		trace_t	tr;
		UTIL_TraceLine( pos, g_pView->MainViewOrigin(), MASK_OPAQUE, NULL, COLLISION_GROUP_NONE, &tr );
...
{
	int viewId = (int)g_pView->CurrentViewID();
...
void FX_Explosion( Vector& origin, Vector& normal, char materialType )
{
	...
	pTraceMaterial = engineClient->TraceLineMaterialAndLighting(engineClient->GetWorldModel(), origin, normal * -16.0f, diffuseColor, baseColor );
