...
BEGIN_PREDICTION_DATA_NO_BASE( C_BaseEntity )
	...
	DEFINE_PRED_FIELD_TOL( m_vecVelocity.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE, 0.5f ),
//	DEFINE_PRED_FIELD( m_fEffects, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_nRenderMode.m_Value, FIELD_CHARACTER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_nRenderFX.m_Value, FIELD_CHARACTER, FTYPEDESC_INSENDTABLE ),
...
//	DEFINE_PRED_FIELD( m_flSimulationTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_fFlags.m_Value, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD_TOL( m_vecViewOffset.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE, 0.25f ),
	DEFINE_PRED_FIELD( m_nModelIndex.m_Value, FIELD_SHORT, FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX ),
	DEFINE_PRED_FIELD( m_flFriction.m_Value, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_iTeamNum.m_Value, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_hOwnerEntity.m_Value, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE ),

//	DEFINE_FIELD( m_nSimulationTick, FIELD_INTEGER ),

	DEFINE_PRED_FIELD( m_hNetworkMoveParent.m_Value, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE ),
...
//	DEFINE_PRED_FIELD( m_pMovePrevPeer, FIELD_EHANDLE ),
	...
	DEFINE_PRED_FIELD_TOL( m_vecNetworkOrigin.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE, coordTolerance ),
	DEFINE_PRED_FIELD( m_angNetworkAngles.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE | FTYPEDESC_NOERRORCHECK ),
...
//	DEFINE_FIELD( m_hGroundEntity, FIELD_EHANDLE ),
	DEFINE_FIELD( m_nWaterLevel.m_Value, FIELD_CHARACTER ),
...
//	DEFINE_FIELD( m_flLastMessageTime, FIELD_FLOAT ),
	DEFINE_FIELD( m_vecBaseVelocity.m_Value, FIELD_VECTOR ),
...
void C_BaseEntity::SetModelPointer( const IVModel *pModel )
{
	if ( pModel != model )
	{
		DestroyModelInstance();
...
int C_BaseEntity::DrawBrushModel( bool bDrawingTranslucency, int nFlags, bool bTwoPass )
{
	...
	{
		render->DrawBrushModelShadowDepth( this, (IVModel *)model, GetAbsOrigin(), GetAbsAngles(), DepthMode );
	}
	else
	{
		...
		render->DrawBrushModelEx( this, (IVModel *)model, GetAbsOrigin(), GetAbsAngles(), mode );
	}
...
int C_BaseEntity::DrawModel(IVModel* pWorld, int flags )
{
	...
	case mod_studio:
		...
		Warning( "ERROR:  Can't draw studio model %s because %s is not derived from C_BaseAnimating\n",
			GetModel()->GetModelName(), GetClientClass()->GetNetworkName() ? GetClientClass()->GetNetworkName() : "unknown");//model
		break;
	case mod_sprite:
		//drawn = DrawSprite();
...
//#ifndef NO_ENTITY_PREDICTION
	if ( GetPredDescMap() )
	{
		const char *mapname = ClientEntityFactoryDictionary()->GetMapClassName( GetPredDescMap()->dataClassName );
...
{
	C_BaseEntity *ent = (C_BaseEntity*)ClientEntityFactoryDictionary()->Create(className,-1,-1);
	if ( ent )
	{
		return ent;
	}
...
CON_COMMAND( cl_sizeof, "Determines the size of the specified client class." )
{
	...
	int size = ClientEntityFactoryDictionary()->GetEntitySize( args[ 1 ] );
