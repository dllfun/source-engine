...
void CGlowOverlay::UpdateSkyGlowObstruction( float zFar, bool bCacheFullSceneState )
{
	...
	if ( PixelVisibility_IsAvailable() )
	{
		...
		Vector pos = g_pView->CurrentViewOrigin() + m_vDirection * zFar * 0.99f; //9f;
	...
	// Trace a ray at the object.
	trace_t trace;
	UTIL_TraceLine(g_pView->CurrentViewOrigin(), g_pView->CurrentViewOrigin() + (m_vDirection*MAX_TRACE_LENGTH),
...
void CGlowOverlay::UpdateGlowObstruction( const Vector &vToGlow, bool bCacheFullSceneState )
{
	...
	if ( PixelVisibility_IsAvailable() )
	{
		if ( m_bInSky )
		{
			...
			Vector pos = g_pView->CurrentViewOrigin() + m_vDirection * (pViewSetup->zFar * 0.999f);
			pixelvis_queryparams_t params;
	...
	if ( m_bInSky )
	{
		// Trace a ray at the object.
		trace_t trace;
		UTIL_TraceLine(g_pView->CurrentViewOrigin(), g_pView->CurrentViewOrigin() + (vToGlow*MAX_TRACE_LENGTH),
			CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE, &trace );
	...
	if ( bFade )
	{
		...
		{
			m_flGlowObstructionScale -= gpGlobals->GetFrameTime() / cl_sun_decay_rate.GetFloat();
	...
	Vector &vRight )
{
	...
	vBasePt = g_pView->CurrentViewOrigin() + vToGlow * flOverlayDist;
