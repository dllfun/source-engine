	...
	case SPR_VP_PARALLEL:
		{
			...
			for (i=0 ; i<3 ; i++)
			{
				up[i]		= g_pView->CurrentViewUp()[i];
				right[i]	= g_pView->CurrentViewRight()[i];
				forward[i]	= g_pView->CurrentViewForward()[i];
			}
		}
		break;
	...
	case SPR_VP_PARALLEL_UPRIGHT:
		{
			...
			// vectors and starts to approach an undefined state, so we don't draw if
			// the two vectors are less than 1 degree apart
			dot = g_pView->CurrentViewForward()[2];	// same as DotProduct (vpn, r_spritedesc.g_vecVUp) because
			//  r_spritedesc.vup is 0, 0, 1
			if ((dot > 0.999848f) || (dot < -0.999848f))	// cos(1 degree) = 0.999848
				return;
			up[0] = 0;
			up[1] = 0;
			up[2] = 1;
			right[0] = g_pView->CurrentViewForward()[1];
			// CrossProduct (r_spritedesc.vup, vpn,
			right[1] = -g_pView->CurrentViewForward()[0];	//  r_spritedesc.vright)
			right[2] = 0;
			VectorNormalize (right);
			forward[0] = -right[1];
			forward[1] = right[0];
	...
	case SPR_VP_PARALLEL_ORIENTED:
		{
			...
			{
				forward[i] = g_pView->CurrentViewForward()[i];
				right[i] = g_pView->CurrentViewRight()[i] * cr + g_pView->CurrentViewUp()[i] * sr;
				up[i] = g_pView->CurrentViewRight()[i] * -sr + g_pView->CurrentViewUp()[i] * cr;
			}
		}
		break;
