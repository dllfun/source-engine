...
void CPointTeleport::Activate( void )
{
	...
	{
		CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, STRING( m_target ) );
		if ( pTarget )
		{
...
void CPointTeleport::InputTeleport( inputdata_t &inputdata )
{
	...
	CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, STRING( m_target ), this, inputdata.pActivator, inputdata.pCaller );
	if ( pTarget == NULL )
		return;
