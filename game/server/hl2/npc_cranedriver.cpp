...
void CNPC_CraneDriver::InputForcePickup( inputdata_t &inputdata )
{
	...
	if ( iszPickupName != NULL_STRING )
	{
		...
		m_hCrane->TurnMagnetOff();
		m_hPickupTarget = gEntList.FindEntityByName( NULL, STRING( iszPickupName ), NULL, inputdata.pActivator, inputdata.pCaller );
...
void CNPC_CraneDriver::InputForceDrop( inputdata_t &inputdata )
{
	...
	{
		CBaseEntity *pEntity = gEntList.FindEntityByName( NULL, STRING( iszDropName ), NULL, inputdata.pActivator, inputdata.pCaller );
		if ( !pEntity )
		{
