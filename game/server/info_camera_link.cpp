...
{
	CInfoCameraLink *pInfoCameraLink = (CInfoCameraLink*)engineServer->CreateEntityByName( "info_camera_link" );
	if ( !pInfoCameraLink )
		return NULL;
...
void PointCameraSetupVisibility( CBaseEntity *pPlayer, int area, unsigned char *pvs, int pvssize )
{
	...
	for ( intp i = g_InfoCameraLinkList.Head(); i != g_InfoCameraLinkList.InvalidIndex(); i = nNext )
	{
		...
		if ( !pTargetEnt->NetworkProp()->IsInPVS( pPlayer->NetworkProp()->GetEdict(), pvs, pvssize ) )
			continue;
