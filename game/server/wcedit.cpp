...
void NWCEdit::CreateAINode( CBasePlayer *pPlayer )
{
	...
	if (bPositionValid)
	{
		...
		else if (status == Editor_OK)
		{
			...
			if (g_pAINetworkManager->GetEditOps()->m_bAirEditMode)
			{
				pNodeEnt = (CNodeEnt*)engineServer->CreateEntityByName("info_node_air");
			}
			else
			{
				pNodeEnt = (CNodeEnt*)engineServer->CreateEntityByName("info_node");
			}
...
void NWCEdit::DestroyAILink( CBasePlayer *pPlayer )
{
	...
	if (pAILink)
	{
		...
		else if (status == Editor_OK)
		{
			...
			CAI_DynamicLink* pNewLink	= (CAI_DynamicLink*)engineServer->CreateEntityByName("info_node_link" );;
