...
void CEnvSoundscapeProxy::Activate()
{
	...
	{
		CBaseEntity *pEntity = gEntList.FindEntityByName( NULL, STRING( m_MainSoundscapeName ) );
		if ( pEntity )
		{
...
void CEnvSoundscape::WriteAudioParamsTo( audioparams_t &audio )
{
	...
	for ( int i = 0; i < ARRAYSIZE(m_positionNames); i++ )
	{
		if ( m_positionNames[i] != NULL_STRING )
		{
			...
			CBaseEntity *pEntity = gEntList.FindEntityByName( NULL, STRING( m_positionNames[i] ), this, this );
			if ( pEntity )
			{
				audio.localBits |= 1<<i;
...
{
	m_hSoundscape = dynamic_cast< CEnvSoundscapeTriggerable* >( gEntList.FindEntityByName( NULL, STRING( m_SoundscapeName ) ) );
	BaseClass::Activate();
}
