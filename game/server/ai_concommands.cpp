...
void CC_AI_Hull( const CCommand &args )
{
	...
	else
	{
		...
		if ( !pEnt )
		{
			...
			pEnt = (CAI_BaseNPC *)engineServer->CreateEntityByName( args[1] );
			if ( !pEnt )
			{
...
void CC_NPC_Create( const CCommand &args )
{
	...
	CAI_BaseNPC *baseNPC = dynamic_cast< CAI_BaseNPC * >(engineServer->CreateEntityByName(args[1]) );
	if (baseNPC)
	{
...
CON_COMMAND(npc_thinknow, "Trigger NPC to think")
{
	...
	if ( pEntity )
	{
		...
		if (pNPC)
		{
			...
			pNPC->SetNextThink( gpGlobals->GetCurTime() );
		}
	}
}
