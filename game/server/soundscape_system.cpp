...
CON_COMMAND(soundscape_flush, "Flushes the server & client side soundscapes")
{
	...
	if (engineServer->IsDedicatedServer() )
	{
		...
		for ( int i = 1; i <= gpGlobals->GetMaxClients(); i++ )
		{
			...
			if ( pSendToPlayer )
				engineServer->ClientCommand( pSendToPlayer->NetworkProp()->GetEdict(), "cl_soundscape_flush\n" );
		}
	}
	else
	{
		engineServer->ClientCommand( pPlayer->NetworkProp()->GetEdict(), "cl_soundscape_flush\n" );
	}
}
...
void CSoundscapeSystem::AddSoundscapeFile( const char *filename )
{
	...
	if ( pKeyValuesData->LoadFromFile(g_pFileSystem, filename, "GAME" ) )
	{
...
bool CSoundscapeSystem::Init()
{
	...
	if( manifest->LoadFromFile(g_pFileSystem, SOUNDSCAPE_MANIFEST_FILE, "GAME" ) )
	{
