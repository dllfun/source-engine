...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( gpGlobals->GetMaxClients() > 1 && breakable_multiplayer.GetBool() )
	{
		...
		case MULTIPLAYER_BREAK_CLIENTSIDE:
			g_pTESystem->PhysicsProp( filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin(), GetAbsAngles(), velocity, true, GetEffects() );
			break;
...
class CPhysicsPropMultiplayer : public CPhysicsProp, public IMultiplayerPhysics
{
	...
	DECLARE_SERVERCLASS();
	DECLARE_SEND_TABLE_ACCESS(DT_PhysicsPropMultiplayer);
	DECLARE_DATADESC();
