...
{ 
	...
	{
		g_NpcTimeThisFrame += engineServer->Time() - g_StartTimeCurThink;
	}
}
...
{
#ifdef HL2_DLL
	CNPC_Bullseye *pTarget = (CNPC_Bullseye*)engineServer->CreateEntityByName( "npc_bullseye" );
...
void CAI_BaseNPC::StartScriptedNPCInteraction( CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction, Vector vecOtherOrigin, QAngle angOtherAngles )
{
	...
	if ( pOtherNPC )
	{
		pTheirSequence = (CAI_ScriptedSequence*)engineServer->CreateEntityByName( "scripted_sequence" );
