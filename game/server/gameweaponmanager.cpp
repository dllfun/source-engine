...
{
	CGameWeaponManager *pManager = (CGameWeaponManager *)engineServer->CreateEntityByName( "game_weapon_manager");
...
void CGameWeaponManager::Think()
{
	...
	for ( i = 0; i < candidates.Count() && surplus > 0; i++ )
	{
		...
		if ( gpGlobals->GetMaxClients() == 1 )
		{
			...
			// more than once.
			if( !UTIL_FindClientInPVS( pCandidate->NetworkProp()->GetEdict()) )
			{
				fRemovedOne = true;
			}
