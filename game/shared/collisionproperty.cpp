...
BEGIN_PREDICTION_DATA_NO_BASE( CCollisionProperty )
	...
	DEFINE_PRED_FIELD( m_vecMinsPreScaled.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_vecMaxsPreScaled.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_vecMins.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_vecMaxs.m_Value, FIELD_VECTOR, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_nSolidType.m_Value, FIELD_CHARACTER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_usSolidFlags.m_Value, FIELD_SHORT, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_triggerBloat.m_Value, FIELD_CHARACTER, FTYPEDESC_INSENDTABLE ),
...
#else
	if (GetOuter()->NetworkProp()->HasEdict()) {
...
void CCollisionProperty::UpdateServerPartitionMask( )
{
	...
	if ( !m_pOuter->NetworkProp()->GetEdict())
		return;
...
void CCollisionProperty::MarkPartitionHandleDirty()
{
	...
	if ( !m_pOuter->NetworkProp()->HasEdict() )
		return;
	if (!m_pOuter->NetworkProp()->entindex())
		return;
