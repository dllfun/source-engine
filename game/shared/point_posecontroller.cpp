...
IMPLEMENT_SERVERCLASS(CPoseController, DT_PoseController, DT_BaseEntity)
...
{
	int iPropNum = 0;
	CBaseEntity *pEnt = gEntList.FindEntityByName( NULL, STRING( m_iszPropName ) );
	...
	while ( pEnt && iPropNum < MAX_POSE_CONTROLLED_PROPS )
	{
		...
		pEnt = gEntList.FindEntityByName( pEnt, STRING( m_iszPropName ) );
	}
