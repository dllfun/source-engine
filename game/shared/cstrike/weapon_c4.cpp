...
END_PREDICTION_DATA()
	...
	void CPlantedC4::Precache()
	{
		...
		engineServer->ForceModelBounds( PLANTED_C4_MODEL, Vector( -7, -13, -3 ), Vector( 9, 12, 11 ) );
	...
	{
		CPlantedC4 *pGrenade = dynamic_cast< CPlantedC4* >(engineServer->CreateEntityByName( "planted_c4" ) );
		if ( pGrenade )
		{
			vecAngles[0] = 0;
			vecAngles[2] = 0;
...
BEGIN_PREDICTION_DATA( CC4 )
	DEFINE_PRED_FIELD( m_bStartedArming.m_Value, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_bBombPlacedAnimation.m_Value, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_fArmedTime.m_Value, FIELD_FLOAT, FTYPEDESC_INSENDTABLE )
END_PREDICTION_DATA()
#endif
...
void CC4::ItemPostFrame()
{
	...
	bool CC4::ShouldRemoveOnRoundRestart()
	{
		...
		if ( pPlayer && pPlayer->GetActiveWeapon() == this )
			engineServer->ClientCommand( pPlayer->NetworkProp()->GetEdict(), "lastinv reset\n" );
		return true;
	}
